    global main                        ; A global label to be declared for the linker ( GNU LD )
	extern ft_write

    ;section .text                           ; Section to put code
main:                                  ; Main global label ( Linker entry point )
    mov edi, 1                          ; STDOUT File descriptor, CC first arg on rdi
    mov esi, message                    ; Address of the string to write, CC 2nd arg on rsi
    mov edx, 13                         ; Length of the string, CC 3rd on rdx
    call    ft_write 
...
message:
    db "Hello World !", 10      ; Define string to the name "message" and 10 is new line ascii code ( db = Define Bytes )
